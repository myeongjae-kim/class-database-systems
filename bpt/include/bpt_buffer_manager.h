...
typedef struct __frame_object {
  ...
  bool is_dirty;
  int32_t pin_count;
  struct __buffer_object * LRU_next; // go to next element.
...
typedef struct buf_mgr {
  ...
  frame_object_t * LRU_queue_head;

  struct __page * (*request_page)(
  ...
  int64_t (*page_alloc)(const int32_t table_id);
  int64_t (*page_free)(const int32_t table_id, const int64_t page_number);
} buf_mgr_t;
